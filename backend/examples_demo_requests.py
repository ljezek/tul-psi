...
async def main() -> None:
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        reqs = [
            client.get(f"{base}/projects", headers={"X-Client-Type": "mobile"}),
            client.get(f"{base}/projects?subject=PSI", headers={"X-Client-Type": "web"}),
            client.get(f"{base}/projects?academic_year=2024/25", headers={"X-Client-Type": "api"}),
            client.get(f"{base}/health/ready", headers={"X-Client-Type": "api"}),
        ]
