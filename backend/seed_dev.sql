...
-- VALUE to fix IDs.
-- Sequences for all surrogate-PK tables are reset at the end of this file.
--
-- Edge cases covered:
...
-- ============================================================================
-- Reset sequences so auto-generated IDs do not collide with seeded rows.
-- These tables use OVERRIDING SYSTEM VALUE above, which bypasses the sequence,
-- leaving it at its initial value of 1. Without this reset, the first INSERT
-- after seeding would attempt id=1 (or the sequence's next value) and fail
-- with a unique-constraint violation on the primary key.
-- ============================================================================
...
SELECT setval(pg_get_serial_sequence('project_member', 'id'),    (SELECT MAX(id) FROM project_member));
SELECT setval(pg_get_serial_sequence('course_evaluation', 'id'), (SELECT MAX(id) FROM course_evaluation));
