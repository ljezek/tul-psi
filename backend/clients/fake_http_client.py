...
from observability.metrics import outbound_http_requests_total, outbound_http_latency_ms
...
async def call_backend_service(service_name: str, operation: str, delay_ms: int) -> dict[str, str]:
    ...
    with tracer.start_as_current_span("http.client") as span:
        ...
        if outbound_http_requests_total:
            outbound_http_requests_total.add(1, attributes=attrs)
        if outbound_http_latency_ms:
            outbound_http_latency_ms.record(elapsed_ms, attributes=attrs)
