...
Links your GitHub repository to the Azure App.
...
1.  **Trigger Infrastructure:** Go to **Actions > Infrastructure Deployment** and run it for `dev`. 
    *   Bicep will adopt the `id-spc-shared-db-setup` identity.
    *   The `deploymentScript` will automatically create the DB roles for `dev`.
...
- **Scale-to-Zero:** The backend is configured with `minReplicas: 0`.
- **Permission Split:** The Migrator identity has DDL rights, while the App identity is restricted to DML.
