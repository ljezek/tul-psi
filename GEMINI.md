# Gemini Instructions — Student Projects Catalogue (SPC)
...
- **No duplicated validation logic**: when the same field validator (e.g., checking that an e-mail belongs to `@tul.cz`) is needed in more than one Pydantic model, extract it into a module-level helper function and call it from each `@field_validator` method. Never copy-paste validator bodies between models.
